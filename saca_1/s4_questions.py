...
class S4_Questions(Frame):
    ...
    def _show_q(self):
        ...
        is_yo_nav = self.app.lang.get() == "Yolngu Matha"
        q_label = f"Dhäruk {self._step+1} / {len(self.QUESTIONS)}" if is_yo_nav else f"Question {self._step+1} of {len(self.QUESTIONS)}"
        Label(nav_row,
              text=q_label,
    ...
    # ── Helpers ──────────────────────────────────────────────────
    def _q_title(self, parent, text, sub, yo_text=None, yo_sub=None):
        """Render question title — shows Yolnu Matha below English if selected."""
        is_yo = self.app.lang.get() == "Yolngu Matha"
        Label(parent, text=text,
              font=("Segoe UI", 28, "bold"),
              fg=TXT1, bg=BG_CARD).pack(pady=(14, 2))
        if is_yo and yo_text:
            Label(parent, text=yo_text,
                  font=("Segoe UI", 16, "italic"),
                  fg=OCHRE, bg=BG_CARD).pack(pady=(0, 2))
        Label(parent, text=sub,
              font=("Segoe UI", 14),
              fg=TXT3, bg=BG_CARD).pack(pady=(0, 4))
        if is_yo and yo_sub:
            Label(parent, text=yo_sub,
                  font=("Segoe UI", 12, "italic"),
                  fg=TXT3, bg=BG_CARD).pack(pady=(0, 10))
        else:
            Label(parent, bg=BG_CARD).pack(pady=4)
    ...
    def _draw_next_btn(self, parent):
        ...
        is_yo = self.app.lang.get() == "Yolngu Matha"
        if is_yo:
            lbl = "DJÄMA  →" if is_last else "NHUMA  →"
        else:
    ...
    def _render_when(self, parent, card_w):
        self._q_title(parent,
                      ...
                      yo_text="Nhaltjan warray ga djäma?",
                      yo_sub="Dhuwali (today)  ·  Warray (yesterday)  ·  Rua dhuwala (2 days)")
        row = Frame(parent, bg=BG_CARD)
        row.pack(pady=12)
        is_yo = self.app.lang.get() == "Yolngu Matha"
        OPTIONS = [
            ("Dhuwali"      if is_yo else "Today",       0,  "#4ADE80"),
            ("Warray"       if is_yo else "Yesterday",   1,  "#60A5FA"),
            ("Rua dhuwala"  if is_yo else "2–7 days",    4,  "#FB923C"),
            ("Lurrkun+"     if is_yo else "Over a week", 10, "#EF4444"),
        ]
        sel_idx = [0]
    ...
    def _render_severity(self, parent, card_w):
        self._q_title(parent,
                      ...
                      yo_text="Nhaltjan ŋuli nhe dhukarr?",
                      yo_sub="1 = mäkitj (mild)  ·  10 = dhukarr djorra (worst pain)")
        row = Frame(parent, bg=BG_CARD)
        row.pack(pady=12)
        LEVELS = [
            (1,  "ok",       "1\nVery Mild"),
    ...
    def _render_daily_activity(self, parent, card_w):
        ...
        is_yo_da = self.app.lang.get() == "Yolngu Matha"
        self._q_title(parent,
                      "Can you do your daily activities?",
                      "Eating  ·  Walking  ·  Looking after yourself  ·  Normal life",
                      yo_text="Nhuma djäma nhe djäma?",
                      yo_sub="Yaka (no) · Dhiyaŋu (a little) · Ŋuli (mostly) · Ŋayi (yes fully)")
        ...
        OPTIONS = [
            ("✓", "Ŋayi"      if is_yo_da else "Yes, fully",  "Djäma normal\nŋayi"    if is_yo_da else "Doing everything\nnormally", "#4ADE80", 4),
            ("~", "Ŋuli"      if is_yo_da else "Mostly yes",  "Ŋuli djäma\ndhiyaŋu"   if is_yo_da else "Slowing down\na little",    "#FCD34D", 3),
            ("!", "Dhiyaŋu"   if is_yo_da else "Struggling",  "Dhiyaŋu djäma\nyolŋu"  if is_yo_da else "Hard to eat,\nwalk or move", "#FB923C", 2),
            ("✗", "Yaka"      if is_yo_da else "No, cannot",  "Yaka djäma\nnhuma"     if is_yo_da else "Lying down,\nneed help",    "#EF4444", 1),
        ]
    ...
    def _render_fever_yn(self, parent, card_w):
        ...
        is_yo_f = self.app.lang.get() == "Yolngu Matha"
        self._q_title(parent,
                      "Do you have a fever?  🌡",
                      "Is your body hot?  ·  Do you feel hotter than normal?",
                      yo_text="Nhe märr dharpa-wuku?",
                      yo_sub="Yaka märr (no fever) · Ŋuli warm · Märr (fever) · Märr djorra (high fever)")
        ...
        OPTIONS = [
            ...
            ("Yaka märr"   if is_yo_f else "No fever",   "Ŋuli normal"     if is_yo_f else "I feel normal",       "#4ADE80", 37.0, False),
            ("Ŋuli warm"   if is_yo_f else "Warm",       "Ŋuli dharpa"     if is_yo_f else "Body feels a bit hot", "#FCD34D", 37.6, True),
            ("Märr"        if is_yo_f else "Fever",      "Dharpa-wuku"     if is_yo_f else "Clearly feverish",     "#FB923C", 38.5, True),
            ("Märr djorra" if is_yo_f else "High fever", "Djorra dharpa"   if is_yo_f else "Very hot / burning",   "#EF4444", 39.5, True),
        ]
        sel_idx = [0]
    ...
    def _render_sick_dizzy(self, parent, card_w):
        is_yo_sd = self.app.lang.get() == "Yolngu Matha"
        """Q5 — Feeling sick or dizzy? Two Yes/No cards side by side."""
        self._q_title(parent,
                      "Feeling sick or dizzy?  🤢",
                      "Nausea · Vomiting · Spinning · Light-headed",
                      yo_text="Nhe wayin bäpa buku-wuku?",
                      yo_sub="Yaka (no) · Wayin (sick) · Buku-wuku (dizzy) · Bukmak (both)")
        ...
        OPTIONS = [
            ...
            ("✓", "Yaka"       if is_yo_sd else "No",           "Yaka wayin"     if is_yo_sd else "Feeling OK",         "#4ADE80", False, False),
            ("~", "Wayin"      if is_yo_sd else "Sick stomach", "Butu wayin"     if is_yo_sd else "Nausea or vomiting", "#FCD34D", True,  False),
            ("!", "Buku-wuku"  if is_yo_sd else "Dizzy",        "Marrtji buku"   if is_yo_sd else "Spinning or faint",  "#FB923C", False, True),
            ("✗", "Bukmak"     if is_yo_sd else "Both",         "Wayin ga buku"  if is_yo_sd else "Sick AND dizzy",     "#EF4444", True,  True),
        ]
        sel_idx = [0]
        CW, CH = 185, 200
    ...
    def _render_breath(self, parent, card_w):
        is_yo_br = self.app.lang.get() == "Yolngu Matha"
        self._q_title(parent,
                      "Can you breathe OK? 💨",
                      "1 = Breathing fine  ·  10 = Cannot breathe",
                      yo_text="Nhe dhu-wuku?",
                      yo_sub="1 = fine · 10 = dhu-wuku (cannot breathe at all)")
        row = Frame(parent, bg=BG_CARD)
        row.pack(pady=12)
        OPTIONS = [
            (1,  "ok",       "1\n" + ("Fine"     if is_yo_br else "Fine")),
            (3,  "mild",     "3\n" + ("Ŋuli"      if is_yo_br else "Slight")),
            (5,  "moderate", "5\n" + ("Dhiyaŋu"   if is_yo_br else "Moderate")),
            (7,  "bad",      "7\n" + ("Dhukarr"   if is_yo_br else "Difficult")),
            (9,  "severe",   "9\n" + ("Djorra"    if is_yo_br else "V. Hard")),
            (10, "extreme",  "10\n" + ("Dhu-wuku" if is_yo_br else "Cannot")),
        ]
        sel_idx = [0]
        self.app.breathe_v.set(1)
        cvs = {}
